 ...
 *
 * True end-to-end test that runs in a real browser using Playwright.
 ...
 *
 * Run with: `npx playwright test e2e-01`
 * Or: `npm run test:e2e:playwright`
 */
...
test("E2E-01: Student access request - full flow", async ({ page }) => {
  ...
  await page.goto("http://localhost:5173/admin/access");
  ...
  // Step 10: Student signs in (simulating magic link redirect)
  await page.goto("http://localhost:5173/login?token=mock-magic-link-token");
...
test("E2E-01: Role-based dashboard redirects", async ({ page }) => {
  ...
  await page.goto("http://localhost:5173/login?token=student-token");
  await expect(page).toHaveURL(/\/student/);

  // Teacher redirects to teacher dashboard
  await page.goto("http://localhost:5173/login?token=teacher-token");
  await expect(page).toHaveURL(/\/teacher/);

  // Admin redirects to admin dashboard
  await page.goto("http://localhost:5173/login?token=admin-token");
