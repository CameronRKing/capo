/**
 * E2E-Reports: Reports & Results Viewing (Browser Mode)
 *
...
class StudentReportsPage {
  ...
  static async selectQuarter(page: any, quarter: string): Promise<void> {
    const quarterButton = page.locator(`button:has-text("${quarter}")`);
    await quarterButton.click();
    await page.waitForTimeout(500);
  }

  static async selectReportType(page: any, type: "Financial" | "Hiring" | "Performance"): Promise<void> {
    const tab = page.locator(`button:has-text("${type}")`);
    await tab.click();
    await page.waitForTimeout(500);
  }

  static async getFinancialSummary(page: any): Promise<{
    sales: string;
    grossMargin: string;
    netIncome: string;
  }> {
  ...
  static async hasReports(page: any): Promise<boolean> {
...
class TeacherReportsPage {
  ...
  static async selectCompany(page: any, companyName: string): Promise<void> {
    const select = page.locator('select').first();
    await select.selectOption({ label: companyName });
    await page.waitForTimeout(500);
  }

  static async selectAllCompanies(page: any): Promise<void> {
    const select = page.locator('select').first();
  ...
  static async getDisplayedCompanies(page: any): Promise<string[]> {
  ...
  static async getAggregatedReportCount(page: any): Promise<number> {
...
class AdminResultsPage {
  ...
  static async getCompilationHistory(page: any): Promise<Array<{
    status: string;
    quarter: string;
    phase: string;
  ...
  static async hasCompilations(page: any): Promise<boolean> {
    const emptyState = page.locator('text=No compilations found');
    return (await emptyState.count()) === 0;
  }

  static async clickTriggerCompilation(page: any): Promise<void> {
...
/**
 * Test 1.1: Student can access their reports page
 */
...
/**
 * Test 1.2: Student can navigate between quarters
 */
...
/**
 * Test 1.3: Student can switch between report types
 */
...
/**
 * Test 1.4: Student financial report displays correctly
 */
...
/**
 * Test 1.5: Student sees only their company's reports
 */
...
/**
 * Test 2.1: Teacher can access their reports page
 */
...
/**
 * Test 2.2: Teacher can view all companies aggregated
 */
...
/**
 * Test 2.3: Teacher can filter by company
 */
...
/**
 * Test 2.4: Teacher can view reports by quarter
 */
...
/**
 * Test 2.5: Teacher can switch between report types
 */
test("E2E-Reports: Teacher report type tabs work correctly", async ({ page }) => {
  ...
  // All tabs visible
  const tabs = page.locator('button').filter(async (btn: any) => {
    const text = await btn.textContent();
    return ["Financial", "Hiring", "Performance"].includes(text || "");
  });
...
/**
 * Test 3.1: Admin can access results page
 */
...
/**
 * Test 3.2: Admin can filter compilation history by game
 */
test("E2E-Reports: Admin can filter compilations by game", async ({ page }) => {
  ...
  const allGamesOption = gameSelector.locator('option').filter(async (opt: any) => {
    const text = await opt.textContent();
    return text?.includes("All games");
  });
...
/**
 * Test 3.3: Admin can filter compilation history by quarter
 */
...
/**
 * Test 3.4: Admin can view compilation history details
 */
...
/**
 * Test 3.5: Admin can access trigger compilation page
 */
...
/**
 * Test 4.1: Student export button exists (PDF)
 */
...
/**
 * Test 4.2: Teacher export button exists (CSV)
 */
...
/**
 * Test 4.3: Admin export button exists (comprehensive data)
 */
...
/**
 * Test 5.1: Financial report displays summary cards
 */
...
/**
 * Test 5.2: Financial report displays line item table
 */
...
/**
 * Test 5.3: Hiring report displays rep outcomes
 */
...
/**
 * Test 5.4: Performance report displays rep data
 */
...
/**
 * Test 6.1: Student can access historical quarters
 */
...
/**
 * Test 6.2: Teacher can view historical compilations
 */
...
/**
 * Test 6.3: Admin can view full compilation history
 */
...
/**
 * Test 7.1: Student cannot access teacher reports
 */
...
/**
 * Test 7.2: Student cannot access admin results
 */
...
/**
 * Test 7.3: Teacher can access admin results (shared permission)
 */
...
/**
 * Test 8.1: Reports page handles missing data gracefully
 */
...
/**
 * Test 8.2: Reports page shows loading state
 */
...
/**
 * Test 8.3: Reports handle missing company assignment
 */
...
/**
 * Test 9.1: Admin can filter by phase (hiring vs leadership)
 */
test("E2E-Reports: Filter results by phase - hiring vs leadership outcomes", async ({ page }) => {
  ...
  const phaseCells = page.locator('td').filter(async (td: any) => {
    const cellIndex = await td.evaluate((el: any) => {
      const parent = el.parentElement;
      return Array.from(parent.children).indexOf(el);
    });
    return cellIndex === 2; // Phase column
  });
...
/**
 * Test 10.1: Report navigation is responsive
 */
...
/**
 * Test 10.2: Report cards have proper structure
 */
test("E2E-Reports: Report cards maintain consistent styling", async ({ page }) => {
  ...
  const hasProperStyling = await reportCard.evaluate((el: any) => {
...
/**
 * Test 10.3: Tables are scrollable on small screens
 */
...
/**
 * Test 11.1: Quarter navigation updates report data
 */
...
/**
 * Test 11.2: Report type navigation preserves quarter selection
 */
